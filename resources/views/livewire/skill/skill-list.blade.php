<div class="py-4 space-y-4">
    ...
    <div class="flex justify-between">
        ...
        <div class="space-x-2 flex items-center">
            ...
            {{-- CREATE NEW PLATFORM --}}
            <x-a.primary href="{{ route('admin.create.skill') }}">
                <x-icon.plus/>
                {{__('New')}}
            </x-a.primary>
        </div>
            ...
            <x-slot name="body">
                ...
                @forelse ($skills as $skill)
                    <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $skill->id }}">
                        ...
                        <x-table.cell>
                            <x-a.link href="{{ route('admin.edit.skill', ['skillId' => $skill->id]) }}">
...
{{--    </x-jet-confirmation-modal>--}}
    ...
    <form wire:submit.prevent="deleteSelected">
        <x-jet-confirmation-modal wire:model="showDeleteModal">
            <x-slot name="title">{{__('Archive announcement')}}</x-slot>
            <x-slot name="content">
                <div class="py-8 text-cool-gray-700">{{__('Which this action you can archive the active or concept announcements. If one of the selected announcements is already archived it will change the status to concept.')}}
                    <strong>{{__ ('Are you sure?') }}</strong></div>
            </x-slot>
            <x-slot name="footer">
