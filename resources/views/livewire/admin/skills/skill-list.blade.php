                ...
                @forelse ($skills as $skill)
                    <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $skill->id }}">
                        ...
                        <x-table.cell class="text-right pr-6">
                            <div x-data="{ tooltip: 'This is crazy!' }">
                            <x-a.link x-tooltip="Edit" class="dark:text-gray-100 dark:hover:text-red-400" href="{{ route('admin.edit.skills', ['skillId' => $skill->id]) }}"><i class="fa-solid fa-pen-to-square"></i></x-a.link>
                            </div>
                        </x-table.cell>
                    </x-table.row>
                @empty
